...
- **出口手动绑定**。「Cookie 池」的编辑弹窗多了「出口绑定」下拉，可以直接指定某个
  账号从哪个代理出去；表格里也补了「出口」列，之前这个绑定关系在面板上完全看不到。

  绑定本身不是新东西：账号第一次请求成功时就会记住当时用的出口，之后一直粘着走
  （同一个号在几十个 IP 之间跳，在 Google 眼里正是账号共享的特征）。以前这个值只能
  由「第一次轮到谁算谁」决定，想改只能停用代理让它自动重绑。

  语义仍是**偏好而非独占**：绑的出口被停用 / 删除 / 熔断后照样自动改绑到别的出口，
  可用性优先。停用中的代理允许绑，方便先配好关系再启用。绑的代理已被删除时下拉里
  会补一个「#N（已删除）」占位 —— 否则只想改个标签的人一提交就把绑定顺手清了。

- **`prefer_anon`：Flash 主动走匿名**（默认关）。开了之后 `gemini-3.7-flash` /
  `gemini-3.6-flash` / `gemini-3.5-flash-lite` 不带 cookie 跑，把账号的配额和风控
  额度全留给非登录不可的请求。

  这三个匿名跑和登录跑拿到的是同一个模型，所以不损失能力。带 `-thinking` 的、
  `3.1 Pro`、生图音乐不受影响 —— 它们匿名等于白跑（思考链 0 字符、Pro 被静默降级成
  Flash-Lite、生图只回一句 "Are you signed in?"）。

  **两条例外仍然取号**：图片输入、超长对话转文本附件。这两个都要往上游传文件，而匿名
  传得上去、一引用就被回 1100。少判这一条的话，开了开关之后 flash 的长对话会从
  「转附件成功」退化成 400。

  跟 `fallback_anon` 正交：那个是 cookie 失效后的**失败兜底**，这个是**主动选择**。
  池子里挑不到号时直接匿名跑，不受 `fallback_anon` 影响 —— 这是用户配置的意图，
  不是故障。

- **`CLAUDE.md` / `AGENTS.md` / `PROJECT_RULES.md`**。给 AI 编码助手看的项目说明与
  经验沉淀：embed 前端改完必须重新编译、不能拿「请求 200」当 cookie 有效性判据、
  登录态才生效的能力清单、账号↔出口粘性、代理池半截读取不能覆盖缓存、限流 slot 按
  出口分桶（含 `rotateAccount` / `handleAdminTest` 两条有意绕过限流器的例外）。
